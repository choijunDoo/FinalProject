...
def preprocess_clevr(features, resolution, apply_crop=False,
  ...
  if get_properties:
    ...
    questions = features["question_answer"]["question"]
    answers = features["question_answer"]["answer"]
    ...
    questions_pad = tf.pad(questions, [[0, 10 - tf.shape(questions)[0],]], "CONSTANT")
    answers_pad = tf.pad(answers, [[0, 10 - tf.shape(answers)[0],]], "CONSTANT")

    features = {
        "image": image,
        "target": properties_pad,
        "question": questions_pad,
        "answer": answers_pad
    }
