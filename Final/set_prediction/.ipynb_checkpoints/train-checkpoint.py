...
def VQA_train_step(batch, tokenizer, encoder, model, optimizer):
    ...
    answer = encoder.transform(decode_answers) ##
    
    with tf.GradientTape() as tape:
        preds = model(batch["image"], training=True)
        loss_value = utils.hungarian_huber_loss(preds, batch["answer"])
...
def main(argv):
  ...
  for _ in range(num_train_steps):
    ...
    loss_value = train_step(batch, model, optimizer) ## Training SLot Attention Classifier
    ...
    vqa_loss = VQA_train_step(batch, tokenizer, model, optimizer)
